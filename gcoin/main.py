...
def deterministic_generate_k(msghash, priv, nonce=None):
    v = b'\x01' * 32
    k = b'\x00' * 32
    priv = encode_privkey(priv, 'bin')
    msghash = encode(hash_to_int(msghash), 256, 32)
    if nonce:
        k = hmac.new(k, v+b'\x00'+priv+msghash+nonce, hashlib.sha256).digest()
    else:
...
def ecdsa_raw_sign(msghash, priv):
    ...
    nonce = encode_privkey(random_key(), 'bin')
    k = deterministic_generate_k(msghash, priv, nonce)
...
def encode_license(license):
    """ Encode license to hex format.
    
    This encoder only uses five fields: `name`, `description`, `issuer`, 
    `fee_collector`, `metadata_link`. All the other fields are hard-coded 
    by this encoder because they have no effect on current gcoin core yet.

    :param license: A license dict.
    ...
    :return: A hex-format encoded licesne.
    """

    def license_field_to_hex(license_field):
        def string_to_hex(string):
            return binascii.hexlify(bytearray(string, 'utf8'))

        def len_to_hex(len_):
            return binascii.hexlify(num_to_var_int(len_))

        result = len_to_hex(len(license_field))
        result += string_to_hex(license_field)
        return result

    def is_license_valid(license):
        max_len = {'name': 32, 'description': 40}
        for key in max_len:
            if len(license[key]) > max_len[key]:
                return False
        return True
    ...
    result = b'72'  # hardcode initial value
    result += b'110110'  # hardcode version
    result += license_field_to_hex(license['name'])
    result += license_field_to_hex(license['description'])
    result += license_field_to_hex(license['issuer'])
    result += b'01'  # hardcode divisibilty to 'true'
    result += b'00000000'  # hardcode fee type to 'fixed'
    result += b'0000000000000000'  # hardcode fee rate
    result += license_field_to_hex(license['fee_collector'])
    result += b'0000000000000000'  # hardcode limit to 0
    result += b'00000000'  # hardcode mint schedule to "free"
    result += b'01'  # hardcode member control to 'true'
    result += license_field_to_hex(license['metadata_link'])
    # hardcode metadata hash
